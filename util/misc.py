...
def update_json(exp_name, acc, path='./save/results.json'):
    # acc 리스트의 각 항목을 % 단위로 변환 후 소수점 2자리 반올림
    acc = [round(a * 100, 2) for a in acc]
    ...
    print('best Score: {} (Acc, Prec, Rec, F1)'.format(acc))
    print('results updated to %s' % path)
'''
def update_json(exp_name, result, path='./save/results.json'):
    """Save either list or dict results into a JSON file."""
    if not os.path.exists(path):
        with open(path, 'w') as f:
            json.dump({}, f)
    ...
    if isinstance(result, list):
        acc = [round(a * 100, 2) for a in result]
        result_dict[exp_name] = acc
        print(f'best Score: {acc} (Acc, Prec, Rec, F1)')

    # ✅ 딕셔너리일 경우 (현재 log_dict 구조)
    elif isinstance(result, dict):
        result["Accuracy"] = round(result["Accuracy"] * 100, 2)
        result["Precision"] = round(result["Precision"] * 100, 2)
        result["Recall"] = round(result["Recall"] * 100, 2)
        result["F1"] = round(result["F1"] * 100, 2)
        result_dict[exp_name] = result
        print(f"✅ JSON updated for {exp_name}: {result}")
